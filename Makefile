...
.PHONY: vscodesetup
vscodesetup: envtest ## Emit environment to run envtest under VSCode (pipe output to tail -1)
	@echo 'KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)"'
...
.PHONY: helm
helm: manifests kustomize kustohelmize yq  ## Generate helm chart
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default --output config/production.yaml
	$(KUSTOHELMIZE) create --from=config/production.yaml \
	    --app-version ${TAG} \
		--version ${CHART_VERSION} \
		--description "Chart for the ECR Secrets Operator" \
		${CHART_DIR}
	tools/helmchart-postprocessor ${CHART_DIR} $(LOCALBIN)
...
$(YQ): $(LOCALBIN)
	curl -Lo $(LOCALBIN)/yq https://github.com/mikefarah/yq/releases/download/v4.33.1/yq_linux_amd64
